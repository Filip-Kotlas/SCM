...
//!   
    ...
    //CRS_Matrix_GPU(CRS_Matrix_GPU const & source);
    CRS_Matrix_GPU(CRS_Matrix_GPU const &) = delete;
    CRS_Matrix_GPU(CRS_Matrix_GPU && source);
    ...
    */
    ...
    std::vector<int> getRowOffset_CPU() const
    {
        int size = this->Nrows() + 1;
        std::vector<int> res( size );
        CHECK_CUDA( cudaMemcpy( res.data(), _d_rowOffsets, size * sizeof(int), cudaMemcpyDeviceToHost ) )
        return res;
    }

    std::vector<int> getColIndices_CPU() const
    {
        int size = this->Nnz();
        std::vector<int> res( size );
        CHECK_CUDA( cudaMemcpy( res.data(), _d_colIndices, size * sizeof(int), cudaMemcpyDeviceToHost ) )
        return res;
    }

    std::vector<double> getValues_CPU() const
    {
        int size = this->Nnz();
        std::vector<double> res( size );
        CHECK_CUDA( cudaMemcpy( res.data(), _d_values, size * sizeof(int), cudaMemcpyDeviceToHost ) )
        return res;
    }
...
private:
    ...
    mutable Vec _d_diag;            //!< diagonal elements of matrix

...
/** @brief  Element-wise vector division w_k = r_k/d_k.
 *
 * @param[in]  n  number of elements
 * @param[out] w  target vector
 * @param[in]  r  source vector
 ...
 *
 */
__global__ void vddiv_elem(int N, double* const w, double const* const r, double const* const d);
 ...
 */
    ...
    Vec(Vec const & source)
    {
        ...
        CHECK_CUDA( cudaMemcpy(ptr, source.data(), nBytes, cudaMemcpyHostToDevice) )
        CHECK_CUSPARSE( cusparseCreateDnVec(&vec, m, ptr, REALTYPE) )
    }
    */

    Vec(Vec && source)
    :ptr( source.ptr ), m( source.m ), vec( source.vec ) 
    {
        source.ptr = nullptr;
        source.vec = nullptr;
    }
...
/** Vector division z_k := x_k/y_k on device
 *
 * @param[out] d_z resulting vector (preallocated)
 * @param[in]  d_x vector
 * @param[in]  d_y vector
 */
void vddiv_gpu(Vec &d_z, Vec const &d_x, Vec const &d_y);
